...
const sleepUntilTimeInZone = async (targetHour24: number, targetMinute: number, timeZoneIANA = 'America/New_York'): Promise<void> => {
    return new Promise((resolve, reject) => {
        ...
        const MAX_SLEEP_DURATION_MS = 10 * 60 * 1000; // 10 minutes in milliseconds

        if (delay > MAX_SLEEP_DURATION_MS) {
            console.log(`Calculated delay of ${delay}ms exceeds maximum of ${MAX_SLEEP_DURATION_MS}ms. Skipping sleep.`);
            resolve();
            return;
        }
  ...
  try {
    const processRequestWithNewBrowser = async (request: BookingRequest): Promise<{ 
      ...
      try {
        ...
        if (process.env.IS_SCHEDULED_RUN == "true" && attempt === 0) {
